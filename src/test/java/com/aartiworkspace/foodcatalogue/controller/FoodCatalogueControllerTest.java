...
class FoodCatalogueControllerTest {
    ...
    void fetchRestauDetailsWithFoodMenu_ShouldReturnOkStatus() {
        ...
        when(foodCatalogueService.fetchFoodCataloguePage(restaurantId)).thenReturn(foodCataloguePage);

        // Act
        ResponseEntity<FoodCataloguePage> response = foodCatalogueController.fetchRestaurantDetailsWithFoodMenu(restaurantId);

        // Assert
        verify(foodCatalogueService, times(1)).fetchFoodCataloguePage(restaurantId);
